...
uses
  ...
  FMX.Objects, FMX.Edit, FMX.ComboEdit, FMX.Controls.Presentation, Inifiles;
...
type
  ...
  TRatioFrame = class(TFrame)
    ...
    procedure pbPreviewMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Single);
    procedure pbPreviewMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Single);
  ...
  public
    ...
    constructor Create(AOwner: TComponent; aWidth, aHeight: Integer;
      AINIFile: TMemIniFile); reintroduce; overload;
    ...
    property SettingRect[AName: string]: TRectF read GetSettingRect
      write SetSettingRect;
...
begin
  ...
  RatioChanged;
  OpenDialog1.Filename := Setting[SizeStr];
  FilenameChanged;
  FSelectRect := SettingRect[SizeStr];
  if FSelectRect.IsEmpty then
  begin
    sbSelectAllClick(nil);
  end;
end;

function TRatioFrame.ImageRect: TRectF;
begin
  result := RectF(0, 0, imPreview.Bitmap.Width / imPreview.Bitmap.BitmapScale,
    imPreview.Bitmap.Height / imPreview.Bitmap.BitmapScale);
...
begin
  result := FINIFile.ReadString('images', AName, '');
end;
...
begin
  ...
  try
    aSize.Delimiter := ',';
    aSize.DelimitedText := FINIFile.ReadString('selections', AName, '');
    ...
    begin
      if (ImageRect.Left + ImageRect.Height * Ratio) > ImageRect.Right then
        result := RectF(ImageRect.Left, ImageRect.Top,
...
procedure TRatioFrame.pbPreviewMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Single);
begin
  case Mode of
    mDraw:
      begin
        ...
        begin
          EndPoint := PointF(StartPoint.X + (Y - StartPoint.Y) * Ratio,
            StartPoint.Y + (Y - StartPoint.Y));
...
begin
  ...
  Label2.Text := 'Select images that are ' + RatioWidth.ToString + 'x' +
    RatioHeight.ToString + ' size or larger for best results';
end;
...
begin
  FINIFile.WriteString('images', AName, Value);
end;
...
begin
  ...
  try
    ...
    FINIFile.WriteString('selections', AName, aSize.DelimitedText);
  finally
    aSize.Free;
  end;
end;
...
begin
  if (ImageRect.Left + ImageRect.Height * Ratio) > ImageRect.Right then
    FSelectRect := RectF(ImageRect.Left, ImageRect.Top,
      ImageRect.Left + ImageRect.Width, ImageRect.Top + ImageRect.Width / Ratio)
  else
    FSelectRect := RectF(ImageRect.Left, ImageRect.Top,
      ImageRect.Left + ImageRect.Height * Ratio,
      ImageRect.Top + ImageRect.Height);
